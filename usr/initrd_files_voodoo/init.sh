...
alias blkrrpart="blockdev --rereadpt /dev/block/mmcblk0"
...
detect_supported_model() {
	...
	for x in /res/mbr_samsung/* /res/mbr_voodoo/* ; do
		if cmp $x /tmp/original.mbr; then
			model=${x##*/}
			if test "${x/voodoo}" = "${x}" ; then
				current_partition_model=samsung
			else
				current_partition_model=voodoo
			fi
			break
		fi
	done
...
say() {
	speexdec --stereo "/res/voices/$1.spx" - | \
		 aplay -c 2 -f S16_LE -r 16000 -Dpcm.AndroidPlayback_Speaker --buffer-size=4096
}
...
if test -f /cache/recovery/command; then
	...
	if test "${recovery_command}" != "${recovery_command/--wipe_data}" ; then
...
if test -n "`find /sdcard/Voodoo/ -iname 'disable*lagfix*'`" ; then
	umount /sdcard
